...
import type { AnalysisDisplayMode, DetailCompareFieldKey, DetailCompareFieldView, DuplicateGroupView, DuplicateItemView, DuplicateKind, ItemDetailFieldKey, RemoveAction, TabView } from '../../core/models/workflow.models';
...
@Component({
  ...
  imports: [FormsModule, OpButtonComponent, OpProgressComponent, SegmentedControlComponent, OpTabsComponent, PlanActionGroupComponent, VaultIconComponent, VaultSelectComponent],
  templateUrl: "./analysis.page.html",
  styleUrls: [
    "./analysis.page.scss",
    "./analysis-filters.scss",
    "./analysis-list.scss",
    "./analysis-drawer.scss",
    "./analysis-drawer-fields.scss",
    "./analysis-actions.scss",
...
export class AnalysisPageComponent implements OnInit {
  ...
  @ViewChild('tagPopoverOverlay') private readonly tagPopoverOverlay?: ElementRef<HTMLElement>;
  ...
  enterTagPopoverTrigger(event: MouseEvent, group: DuplicateGroupView, item: DuplicateItemView): void {
    this.showTagPopover(event, group, item);
  }

  leaveTagPopoverTrigger(event: MouseEvent): void {
    if (!this.isWithinTagPopover(event.relatedTarget)) {
      this.closeTagPopover();
    }
  }

  focusTagPopoverTrigger(event: FocusEvent, group: DuplicateGroupView, item: DuplicateItemView): void {
    this.showTagPopover(event, group, item);
  }

  blurTagPopoverTrigger(event: FocusEvent): void {
    if (!this.isWithinTagPopover(event.relatedTarget)) {
      this.closeTagPopover();
    }
  }

  enterTagPopoverOverlay(): void {
    // The overlay is intentionally a second, contiguous part of the hover region.
  }

  leaveTagPopoverOverlay(event: MouseEvent): void {
    if (!this.isTagPopoverTrigger(event.relatedTarget)) {
      this.closeTagPopover();
    }
  }

  focusTagPopoverOverlay(): void {
    // Focus inside the overlay keeps the corresponding trigger interaction available.
  }

  blurTagPopoverOverlay(event: FocusEvent): void {
    if (!this.isTagPopoverTrigger(event.relatedTarget)) {
      this.closeTagPopover();
    }
  }
  ...
  private isWithinTagPopover(target: EventTarget | null): boolean {
    return target instanceof Node && this.tagPopoverOverlay?.nativeElement.contains(target) === true;
  }

  private isTagPopoverTrigger(target: EventTarget | null): boolean {
    return target instanceof HTMLElement && target.closest('.tag-column-trigger') !== null;
  }
  ...
  itemDecisionValue(item: DuplicateItemView): 'keep' | RemoveAction {
